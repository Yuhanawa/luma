async showConfirm(group_id, email_domains) {
  ...
  if (domainCount > MAX_AUTO_MEMBERSHIP_DOMAINS_LOOKUP) {
    return new Promise(resolve => {
      this.dialog.confirm({
        message: i18n("admin.groups.manage.membership.automatic_membership_user_unknown_count"),
  ...
  try {
    const result = await ajax(`/admin/groups/automatic_membership_count.json`, {
      type: "PUT",
      data
    });
    ...
    if (count > 0) {
      return new Promise(resolve => {
        this.dialog.confirm({
          message: i18n("admin.groups.manage.membership.automatic_membership_user_count", {
            count
          }),
          didConfirm: () => resolve(true),
          didCancel: () => resolve(false)
