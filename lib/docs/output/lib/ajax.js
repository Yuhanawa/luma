function updateCsrfToken() {
  if (!activeCsrfRequest) {
    activeCsrfRequest = ajax("/session/csrf").then(result => Session.currentProp("csrfToken", result.csrf)).finally(() => activeCsrfRequest = null);
  }
...
function ajax() {
  ...
  function performAjax(resolve, reject) {
    ...
    args.success = (data, textStatus, xhr) => {
      ...
      handleLogoff(xhr);
      run(() => {
        Site.currentProp("isReadOnly", !!(xhr && xhr.getResponseHeader("Discourse-Readonly")));
      });
      if (args.returnXHR) {
        data = {
          result: data,
  ...
  if (args.type && args.type.toUpperCase() !== "GET" && url !== getURL("/clicks/track") && !Session.currentProp("csrfToken")) {
